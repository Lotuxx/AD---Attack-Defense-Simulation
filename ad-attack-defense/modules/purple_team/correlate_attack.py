"""
Purple Team — Corrélation attaques simulées / alertes SIEM.
Compare ce qui a été exécuté vs ce que Wazuh a détecté.
Produit un rapport de couverture de détection.
"""
...
def _load_executed_attacks() -> list:
    """Scan /reports/ for red_team_*.txt to find which attacks were run."""
    base = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.dirname(__file__)))), "reports")
    executed = []
    for path in glob.glob(os.path.join(base, "red_team_*.txt")):
        fname = os.path.basename(path)
        for module_path in CORRELATION_MAP:
            short = module_path.replace("red_team.", "").replace("_", "")
            if short in fname.replace("_", ""):
