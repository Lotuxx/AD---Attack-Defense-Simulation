...
============================
MITRE ATT&CK: T1021.002 (SMB/PsExec) | T1047 (WMI)

After initial compromise, attackers move laterally to reach higher-value
targets. This module simulates remote execution across the network using
common lateral movement techniques.

Techniques simulated:
    - PsExec  : Uploads a service binary (PSEXESVC) via SMB, executes commands as SYSTEM
    - WMIExec : Uses Windows Management Instrumentation for stealthier remote execution
    - SMBExec : Executes commands via SMB shares without dropping a binary on disk

Attack flow:
    1. Discover reachable hosts on the subnet (nmap ping sweep)
    2. For each reachable host, attempt execution via PsExec → WMIExec → SMBExec
    3. Execute a test command (whoami) to confirm access
    4. Document access method and SIEM-observable indicators

Detection indicators:
    - Event ID 4624: Network logon (type 3) to target host
    - Event ID 7045: New service installed (PSEXESVC - PsExec fingerprint)
    - Event ID 4688: New process created remotely
    - Wazuh Rules 18104, 60401
...
def run_attack(target: str = "192.168.56.11", domain: str = "domain.local",
    ...
    # Charger credentials depuis config si pas fournis
...
def _try_execution(method: str, target: str, domain: str,
    ...
    auth = _build_auth(domain, user, password, ntlm_hash, target)
    if auth is None:
        return None
...
def _build_auth(domain: str, user: str,
    ...
    if ntlm_hash:
        ...
        return [target_str, "-hashes", full, f"{target}"]
    if password:
        return [f"{target_str}:{password}@{target}"]
    return None
