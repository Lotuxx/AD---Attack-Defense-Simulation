"""
Purple Team — Fetch Wazuh Alerts
==================================
Retrieves and categorises recent security alerts from the Wazuh SIEM API.

Connects to the Wazuh REST API (port 55000), fetches alerts from the
last N minutes above a minimum severity level, then maps each alert's
rule ID to an attack category (Password Spraying, Kerberoasting, etc.).

Output includes:
    - Total alert count per attack category
    - Maximum severity observed per category
    - Agents (machines) generating the most alerts
    - Structured findings for report generation

Falls back to simulated demo data if Wazuh is unreachable, allowing
the module to demonstrate its functionality without a live SIEM.
...
RULE_ATTACK_MAP = {
    ...
    60122: "Password Spraying",
    # Kerberoasting
    60106: "Kerberoasting",      100001: "Kerberoasting",
    100002: "Kerberoasting",
    # LLMNR Poisoning
    17501: "LLMNR Poisoning",
    # Pass-the-Hash
    18107: "Pass-the-Hash",      60301: "Pass-the-Hash",
    # Lateral Movement
    92213: "Lateral Movement",   92057: "Lateral Movement",
